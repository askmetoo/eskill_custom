...
frappe.pages["price-lookup"].on_page_load = function (wrapper) {
  ...
  document
    .getElementsByClassName("page-head-content")[0]
  ...
  constructor(wrapper) {
    ...
    setInterval(() => {
      if (current_item && this.page.match_price_check.value) {
        if (
          this.page.item_field.value ==
          current_item.item_obj[current_item.item_field]
        ) {
          this.page.base_selling_price.set_value(
            current_item.item_obj[current_item.base_rate_field]
          );
        }
      }
    }, 5000);
    ...
    this.page.add_inner_button(__("Refresh"), () => {
      if (!this.page.match_price_check.value) {
        if (!current_item) {
          frappe.msgprint(
            'This lookup window is not linked to a specific item.<br>Please use the "Check Price" button to link to an item.'
          );
        } else {
          this.page.match_price_check.set_value(1);
          this.page.item_field.set_value(
            current_item.item_obj[current_item.item_field]
          );
          this.page.base_selling_price.set_value(
            current_item.item_obj[current_item.base_rate_field]
  ...
  make() {
    ...
    this.page.item_field = this.page.add_field({
      ...
      change() {
        ...
        frappe.call({
          method:
            "eskill_custom.eskill_customisations.page.price_lookup.price_lookup.get_gp_rate",
          args: {
    ...
    this.page.base_selling_price = this.page.add_field({
      ...
      change() {
        if (current_item) {
          if (
            this.value != current_item.item_obj[current_item.base_rate_field]
          ) {
            parent.page.match_price_check.set_value(0);
          }
        }
      ...
      .then((bins) => {
        ...
        for (let i = 0; i < bins.length; i++) {
          ...
          const selling_price = Math.ceil(
            (current_bin.valuation_rate / (1 - this.min_gp_rate)) * TAX_RATE
          );
          const net_price = this.page.base_selling_price.value / TAX_RATE;
          const current_gp_rate =
            (net_price - current_bin.valuation_rate) / net_price;
          ...
          table_rows.push(`
              ...
              current_gp_rate < this.min_gp_rate ? 'style="color: red;"' : ""
            }>
